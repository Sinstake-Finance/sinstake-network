...
let BANKROLL = {
    ...
    "BSC": { 
        StackPlus: {
            ...
            "sinstake" : { token: "0x544Be21b097d99c739D027ed42Edef6a0a98ee2A", stack: "0xfC30ea56548A4Aeee6571bC9D86b7BBEeF4C4EaA",proxy: "0xab2fac3769D285d40216a2B3F95C93e8E439ff74", block: 7413394, timestamp: 1621027367 , symbol: "SIN", padding: 8},
...
function convertEthToSin(e) {
    return 1e18 * e
}

function convertSinToEth(e){
	return e / 1e8;
}
...
function formatSunX(sun, padding) {
    ...
    return (value.trim() == 'NAN') ? formatSunSinstake(0, padding) : value
}


function formatSunSinstakeX(sun){
	let value = numeral(convertSinToEth(sun)).format('0,0.000 a').toUpperCase()
    return (value.trim() == 'NAN') ?  formatSun(0) :  value
}

function formatSunSinstake(sun, padding){
	let value = numeral(convertSinToEth(sun)).format(`0,${parseFloat(0).toFixed(padding)} a`).toUpperCase()
	return (value.trim() == 'NAN') ? formatSunSinstake(0, padding) : value
}

function formatSunSinstakeValue(sun){
	let value = Number.parseFloat(sun/1e10).toFixed(18);
	return (value.trim() == 'NAN') ? formatSunSinstakeValue(0, 18) : value
}
...
async function showWalletInfo() {
    try {
        ...
        $('#walletBalanceValue').text(formatSunSinstake(balance, padding))
    } catch (e) {
        console.error(e)
...
async function showStats() {
    try {
        ...
        $('#contractBalance').text(formatSunSinstakeX(tokenBalance))
        $('#totalSupply').text(formatSunSinstakeX(totalSupply))
        $('#dividendPool').text(formatSunSinstakeX(dividendBalance))
        $('#totalClaims').text(formatSunSinstakeX(totalClaims))
        if (collateral_price != null) {
            $('#contractBalance-usdt').html(` ${approxStr} ${formatSunSinstakeX(tokenBalance * collateral_price)} USD`)
            $('#dividendPool-usdt').html(` ${approxStr} ${formatSunSinstakeX(dividendBalance * collateral_price)} USD`)
            $('#totalSupply-usdt').html(` ${approxStr} ${formatSunSinstakeX(totalSupply * collateral_price)} USD`)
        }
...
async function showUserStats() {
    ...
    let withdrawn = formatSunSinstake(stats[1],padding)
    let reinvested = formatSunSinstake(stats[12], padding)
    ...
    $('#user-bonus').text(formatSunSinstake(balance, padding))
   

    $('#user-vault').text(formatSunSinstake(divs, padding))
...
async function sell() {
    ...
    */
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
		...
		amount = formatSunSinstakeValue(amount);
		amont = (amount <tokens ) ? amount : tokens
                ...
                <div class="card d-flex flex-row mb-3">
                    <div class="d-flex flex-grow-1 min-width-zero">
                        <div class="card-body align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
                            ...
                            <div class="mb-1 text-white w-15 w-xs-100 ">${formatSunSinstake(item.boost, padding)}</div>
