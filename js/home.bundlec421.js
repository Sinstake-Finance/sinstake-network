...
async function main() {
    ...
    let tokens = ['wbnb', 'bdollar', 'binance-bitcoin', 'busd']

    var stack, sumPlayers = 0, sumTVL = 0, sumTxs = 0, bnb_price = 0
    ...
    for(let i=0; i < tokens.length; i++) {
        token = tokens[i]
        stack = new web3js.eth.Contract(ABI_STACK, BANKROLL[BANKROLL.network].StackPlus[token].stack, options)
        try {

            let [totalTxs, players, tokenBalance, totalSupply, dividendBalance, bdo_price] =
                await Promise.all(
                    [
                        ...
                        coingeckoPrice(token)
                    ]
                )

                sumPlayers += parseInt(players)
                sumTVL += parseInt(tokenBalance) * bdo_price
                sumTxs +=  parseInt(totalTxs)
            ...
            let start = BANKROLL[BANKROLL.network].StackPlus[token].timestamp //unix timestamp of start of contract
            ...
            let dripApy = 100 * 365 * 0.02 *  dividendBalance / totalSupply
            $(`#${token}-apy`).text(`${numeral(dripApy + instantApy).format('0.000')} % APY`)
    ...
    if (bnb_price > 0){
        let raise = await ldrive.methods.totalEthDonated().call()
        $('#lde-raise').html(`${formatSun(raise)} BNB raised <small>(${formatSun(raise * bnb_price)} USD)</small>`)
        //let startedOn = await elephant.methods.startedOn().call()
        //let expires = moment.unix(startedOn).add(7, 'days').fromNow()
        $('#lde-expires').text(`Completed Successfully`)
    }

    $('#totalTxs').text(numeral(sumTxs).format('0,0.000 a').toUpperCase())
    $('#getTotalMembers').text(sumPlayers)
    $('#contractBalance').text(`${formatSun(sumTVL)}`)
