...
async function showStats() {
    try {
        ...
        let [totalTxs, participants, tokenBalance] =
            await Promise.all(
                [
                    ldrive.methods.totalTxs().call(),
                    ldrive.methods.participants().call(),
                    ldrive.methods.totalEthDonated().call()
                ]
            )

        $('#totalTxs').text(numeral(totalTxs).format('0,0.000 a').toUpperCase())
        $('#getTotalMembers').text(participants)
...
async function showUserStats() {
    let [available, claimed, donated, balance] =
        await Promise.all(
            [
                ldrive.methods.availableOf(currentAddress).call(),
                ldrive.methods.claimedOf(currentAddress).call(),
                ldrive.methods.donationsOf(currentAddress).call(),
                ldrive.methods.totalEthDonated().call()
            ]
        )


    let stakePercent = donated / balance * 100

    let elephant_balance = await elephant.methods.balanceOf(currentAddress).call()

    $('#user-percentage').text(numeral(stakePercent).format('0.000') + ' %')

    
    $('#user-available').text(formatElephant(available))

    $('#elephant-balance').text(formatElephant(elephant_balance))
   

    $('#user-donated').text(formatSun(donated))
    $('#user-claimed').text((claimed > 0) ? "Claimed" : "");
